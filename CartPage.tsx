    ...
    <Box p={3}>
      <Grid container spacing={2}>
        <Grid item xs={12} md={8}>
          <Card>
            <CardContent>
              <Typography variant="h4" color="secondary">CHECKOUT</Typography>
              {/* <Divider sx={{ my: 2 }} /> */}
              <hr></hr>
              <Typography variant="body1">Total Items: {count}</Typography>
              <Divider sx={{ my: 2 }} />
              {items.map((item) => (
                <Box key={item.id} mb={2}>
                  <Grid container spacing={2} alignItems="center">
                    ...
                    <Grid item xs>
                      ...
                      <AiFillPlusCircle onClick={() => handleAddItem(item.id)} style={{ cursor: 'pointer' }} />
                      <HiMinusCircle onClick={() => handleDelItem(item.id)} style={{ cursor: 'pointer' }} />
                      <Typography variant="body1"><b>Name: </b>{item.model}</Typography>
                      <Typography variant="body1"><b>Price: </b>${item.price}</Typography>
                    </Grid>
                    <Grid item>
                      <Button variant="outlined" color="secondary" onClick={() => handleRemoveFromCart(item.id)}>Remove from cart</Button>
                    </Grid>
                  </Grid>
                  <Divider sx={{ my: 2 }} />
                </Box>
        ...
        <Grid item xs={12} md={4}>
          <Card>
            <CardContent>
              <Typography variant="h5" color="secondary">Estimated Amount</Typography>
              {/* <Divider sx={{ my: 2 }} /> */}
              <hr></hr>
              ...
              {/* <Divider sx={{ my: 2 }} /> */}
              <hr></hr>
              <Typography variant="h6" color="secondary" display="flex" justifyContent="space-between"><b>Estimated Total:</b> ${totalPrice.toFixed(2)}</Typography>
              <Box mt={2}>
